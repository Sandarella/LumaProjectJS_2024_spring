...
test.describe('menBottomWishList.spec', () => {
  ...
  test('_should be a delete item button, a cross', async ({ page }) => {
    await homePage.hoverMenLink();
    const menBottomsPage = await homePage.clickMenBottomsLink();
    const pierceGymShortPage = await menBottomsPage.ckickPierceGymc();
    await page.waitForTimeout(3000);
  ...
  test('_should be an "Add to Cart" button', async ({ page }) => {
    await homePage.hoverMenLink();
    const menBottomsPage =await homePage.clickMenBottomsLink();
    const pierceGymShortPage = await menBottomsPage.ckickPierceGymc();
    await page.waitForTimeout(3000)
    ...
    await expect(pierceGymShortPage.locators.getProductShortTitle()).toBeVisible();
